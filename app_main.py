...
import json
import threading
from current_form import get_current_season
import datetime
...
def past_results():
    ...
    season_start = datetime.datetime(get_current_season(), 7, 1).date().strftime('%Y-%m-%d')
    query = 'SELECT * FROM past_results WHERE Date > "{}"'.format(season_start)
    req_params_raw = request.data
    if req_params_raw:
        req_params = json.loads(req_params_raw)
        query_type = 'AND' if 'against' in req_params else 'OR'

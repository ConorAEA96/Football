...
def add_current_details(from_path, to_path, standings_path, year_available_from):
    ...
    df['HT_3_win_streak'] = df['HT_last_matches'].apply(three_game_win_streak)
    df['HT_5_win_streak'] = df['HT_last_matches'].apply(five_game_win_streak)
    df['HT_3_lose_Streak'] = df['HT_last_matches'].apply(three_game_lose_streak)
    df['HT_5_lose_Streak'] = df['HT_last_matches'].apply(five_game_lose_streak)
    df['AT_3_win_streak'] = df['AT_last_matches'].apply(three_game_win_streak)
    df['AT_5_win_streak'] = df['AT_last_matches'].apply(five_game_win_streak)
    df['AT_3_lose_Streak'] = df['AT_last_matches'].apply(three_game_lose_streak)
    df['AT_5_lose_Streak'] = df['AT_last_matches'].apply(five_game_lose_streak)
    df['HT_5_win_rate'] = df['HT_last_matches'].apply(five_win_rate)
    df['AT_5_win_rate'] = df['AT_last_matches'].apply(five_win_rate)
